...
def poly_feature_creation(df_scaled, list_of_columns, max_degree, interaction_only = True):
    """This function adds to the existing data frame all the possible interactions between the specified columns given 
    a maximum polynomial degree.
    
    Parameters
    ----------
    df_scaled =  the resulting data frame after standardizing the independent numerical variables (target is excluded)
    ...
    Returns:
    ---------- 
    The function returns the original data frame including new columns for each of the interactions."""
